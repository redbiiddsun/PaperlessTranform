...
for page_num, image in enumerate(images):
    ...
    for cnt in contours:
        ...
        roi = (binary[y:y + 10, x:x + 10])
        if (min_w <= w <= max_w and min_h <= h <= max_h) and (0.8 < aspect_ratio <= 1.1) and 100 < area < 1000 and np.sum(roi)/1000 > 8 :
            ...
            option_text = pytesseract.image_to_string(option_roi, lang='tha', config='--psm 6').strip()
